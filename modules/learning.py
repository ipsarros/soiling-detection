...
def train_on_reference_points(df, w_train, ref_points, feats, target, random_state=0):
    ...
    # kmeans = KMeans(n_clusters=100, random_state=0).fit(df_train)
    # labels = kmeans.labels_
    # n_clusters = []
    # dfs = []
    # masks = []
    # for i in range(len(labels)):
    #     masks.append(np.where(labels == i)[0])
    #     n_clusters.append(len(np.where(labels == i)[0]))
    #     if len(masks[i])>0:
    #         q = np.percentile(df_train[target].iloc[masks[i]], 25)
    #         dfs.append(df_train.iloc[masks[i]][df_train.iloc[masks[i]].power > q])
...
def calc_changepoints_one_model(df, dates_rain_start, dates_rain_stop, model, target, feats, w1, w2):
    ...
    errors_all = np.empty(dates_rain_start.size)
    for i in range(dates_rain_start.size):
        ...
        try:
            ...
            errors_all[i] = st.score2(df_ar[target].array, y_pred_ar, df_br[target].array, y_pred_br)
    ...
    return errors_br, errors_ar, errors_all        
...
def calc_changepoints_many_models(df, dates_rain_start, dates_rain_stop, target, feats, w1, w2, w3):
    ...
    for i in range(dates_rain_start.size):
        d1 = dates_rain_start.iloc[i]
        d2 = dates_rain_stop.iloc[i]
        d0 = d1 - pd.Timedelta(days=w2)
        d3 = d2 + pd.Timedelta(days=w3)
        df_ar = df[d2:d3]
        df_br = df[d0:d1]
        try:
            ...
            errors_all[i] = st.score2(df_ar[target].array, y_pred_test, df_br[target].array, y_pred_val)
        except Exception as e:
            print(e)
            errors_ar[i,:] = [np.nan]*6
            errors_br[i,:] = [np.nan]*6
